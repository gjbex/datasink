...
void parseCL(Params *params, int *argc, char **argv[]) {
	...
	while (i < *argc) {
		...
		if (!strncmp((*argv)[i], "-out_file_base", 15)) {
			shiftCL(&i, *argc, *argv);
			argv_str = (*argv)[i];
			if (!1) {
				fprintf(stderr, "### error: invalid value for option '-out_file_base' of type char *\n");
				exit(EXIT_CL_INVALID_VALUE);
			}
			char *tmp;
			int len = strlen(argv_str);
			free(params->out_file_base);
			if (!(tmp = (char *) calloc(len + 1, sizeof(char))))
				errx(EXIT_CL_ALLOC_FAIL, "can not allocate char* field");
			params->out_file_base = strncpy(tmp, argv_str, len + 1);
			i++;
			continue;
		}
...
void parseFileCL(Params *params, char *fileName) {
	...
	while (fgets(line_str, MAX_CL_LINE_LEN, fp)) {
		...
		if (sscanf(line_str, "out_file_base = %[^\n]", argv_str) == 1) {
			if (!1) {
				fprintf(stderr, "### error: invalid value for option '-out_file_base' of type char *\n");
				exit(EXIT_CL_INVALID_VALUE);
			}
			...
			if (!(tmp = (char *) calloc(len + 1, sizeof(char))))
				errx(EXIT_CL_ALLOC_FAIL, "can not allocate char* field");
			params->out_file_base = strncpy(tmp, argv_str, len + 1);
			stripQuotesCL(params->out_file_base);
			continue;
		}
...
void dumpCL(FILE *fp, char prefix[], Params *params) {
	...
	fprintf(fp, "%sout_file_base = '%s'\n", prefix, params->out_file_base);
